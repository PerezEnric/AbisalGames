...
int ModuleFonts::Load(const char* texture_path, const char* characters, uint rows)
{
	...
	App->textures->GetSize(tex, font_width, font_height);
	...
	fonts[id].char_w = font_width / fonts[id].row_chars;// char_w: width of each character
	fonts[id].char_h = font_height / rows; // char_h: height of each character
...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text) const
{
	...
	for (uint i = 0; i < len; ++i)
	{
		...
		charRec.y = 2; //To change if have more rows
